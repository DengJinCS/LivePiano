...
CHUNKSIZE = 2048  # fixed chunk size
...
def getMicrophoneData(p):
    ...
    numpydata = np.frombuffer(data, dtype=np.int16).tolist()
...
"""
path = '/Users/atticus/Music/网易云音乐/kele.mp3'
y,sr = librosa.load(path,sr=None)
s = int(len(y)/CHUNKSIZE)
for i in range(s):
    signal = y[i*CHUNKSIZE:(i+1)*CHUNKSIZE]
